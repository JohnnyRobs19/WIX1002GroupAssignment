...
public class EMPCUSTOMERController implements Initializable {
  ...
  private void loadCSVData() {
    ...
    String custCsvFilePath = "src//backups//customerBackup.csv";
    ...
    try (CSVReader salesCsvReader = new CSVReader(new FileReader(salesCsvFilePath));
        ...
        // Retrieve the list of customers for the logged-in employee from the map
        List<customer> customerList = employeeToCustomerMap.getOrDefault(AppContext.getLoggedInUserId(), new ArrayList<>());
  ...
  private Map<String, List<customer>> connectSalesAndCustomers(List<sales> salesData, List<customer> customerData) {
    ...
    for (sales sale : salesData) {
        // Check if the sale belongs to the logged-in employee
        if (sale.getEmployeeId().equals(loggedInEmployeeId)) {
            ...
            // If a matching customer is found, associate it with the logged-in employee
            if (matchedCustomer != null) {
                employeeToCustomerMap
                        .computeIfAbsent(loggedInEmployeeId, k -> new ArrayList<>())
                        .add(matchedCustomer);
            }
        }
    }
...
private void updateTableView() {
    ...
    if (filteredData.isEmpty()) {
        ...
        showNoCustomersFoundLabel();
    } else {
        // If there are results, hide the message label
        hideNoCustomersFoundLabel();
    }
}

private void showNoCustomersFoundLabel() {
    Label noCustomersFoundLabel = new Label("NO CUSTOMERS FOUND");
    // Customize the label appearance if needed
    // noCustomersFoundLabel.setStyle("-fx-text-fill: red; -fx-font-size: 14;");

    // Set the label as the placeholder
    empCustomerTableView.setPlaceholder(noCustomersFoundLabel);
}

private void hideNoCustomersFoundLabel() {
    // Reset the placeholder (clear the "NO CUSTOMERS FOUND" message)
    empCustomerTableView.setPlaceholder(null);
}
