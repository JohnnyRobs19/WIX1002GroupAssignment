...
public class EMPVEHICLESController implements Initializable {
     @FXML
    private TableView<vehicles> empVehiclesTable;
...
private void applyFilters() {
    ...
    if (filteredData.isEmpty()) {
        ...
        showNoVehiclesFoundLabel();
    } else {
        // If there are results, hide the message label
        hideNoVehiclesFoundLabel();
    }
}

private void showNoVehiclesFoundLabel() {
    Label noVehiclesFoundLabel = new Label("NO VEHICLES FOUND");
    // Customize the label appearance if needed
    // noVehiclesFoundLabel.setStyle("-fx-text-fill: red; -fx-font-size: 14;");

    // Set the label as the placeholder
    empVehiclesTable.setPlaceholder(noVehiclesFoundLabel);
}

private void hideNoVehiclesFoundLabel() {
    // Reset the placeholder (clear the "NO VEHICLES FOUND" message)
    empVehiclesTable.setPlaceholder(null);
}
...
private void updateTableView() {
    ...
    sortedData.comparatorProperty().bind(empVehiclesTable.comparatorProperty());

    // Set the sorted (and filtered) data to the TableView
    empVehiclesTable.setItems(sortedData);
